<template>
  <div>
    ...
    >
      <img v-if="lainxiwomen.img" :src="lainxiwomen.img" class="avatar" style="max-height: 690px;" />
...
export default {
  ...
  methods: {
    ...
    select() {
      let url = "/oppoLianxiwomen/select";
      this.$axios.get(url).then(result => {
    ...
    lainxiwomenBeforeAvatarUpload(file) {
      ...
      client
        ...
        .then(function(result) {
          ...
          _that.$axios .post("/oppoLianxiwomen/sava", _that.lainxiwomen) .then(result => {});
        })
        .catch(function(err) {
          console.log(err);
        });
